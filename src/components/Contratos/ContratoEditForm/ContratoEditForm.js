...
import styles from './ContratoEditForm.module.css'

export function ContratoEditForm(props) {

  const { reload, onReload, contrato, onOpenEditContrato, onToastSuccessContratoMod } = props
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`/api/contratos/contratos?id=${contrato.id}`, {
        ...formData
      })
      onReload()
      onOpenEditContrato()
      onToastSuccessContratoMod()
    } catch (error) {
      console.error('Error actualizando el contrato:', error)
    }
  }
  ...
  return (
    ...
    <>
      ...
      <IconClose onOpenClose={onOpenEditContrato} />
